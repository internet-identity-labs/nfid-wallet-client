...
export class BtcWallet {
  ...
  async getBitcoinAddress(): Promise<string> {
    ...
    const network = "mainnet" == CHAIN_NETWORK ? networks.bitcoin : networks.testnet;
  ...
  async sendSatoshi(targetAddress: string, satoshi: number): Promise<BlockCypherTx> {
  ...
  async computeTransaction(address: string, transactionValue: number, targetAddress: string) {
    ...
    const inputs = json.txs.map((tx: any) => ({
      txid: tx.hash,
      vout: tx.outputs.findIndex((output: any) => output.addresses.includes(address)),
      value: tx.outputs.find((output: any) => output.addresses.includes(address)).value
    }));
