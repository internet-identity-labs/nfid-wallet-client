...
export class NftYumi extends NFTExt {
  ...
  async getDetails(): Promise<NFTDetails> {
    let collectionResponse = await fetch(`https://stat.yuku.app/api/collection/${this.getCollectionId()}`)
...
class NFTYumiDetails extends NFTDetailsImpl {
  ...
  async getTransactions(from: number, to: number): Promise<NFTTransactions> {
    ...
    console.log(`Fetching transactions for token ${this.tokenNumber} from page ${page} with limit ${limit}`);
    ...
    const transactions: TransactionRecord[] = response.data.data
      .map((transaction) => yumiTransactionMapper.toTransactionRecord(transaction))
