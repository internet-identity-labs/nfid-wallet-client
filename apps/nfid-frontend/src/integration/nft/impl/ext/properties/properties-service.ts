import {Category, ParsedData, ValueMapping} from "src/integration/nft/impl/ext/properties/types";
import {TokenProperties} from "src/integration/nft/impl/nft-types";

export class ExtPropertiesService {

  async getProperties(collectionId: string, tokenId: number): Promise<TokenProperties> {
    return this.fetchCollectionData(collectionId).then(this.parseData)
      .then(parsedData => {
  ...
  private mapToTokenProperties({parsedData, tokenId}: { parsedData: ParsedData, tokenId: number }): TokenProperties {
    //for some reason, the token number is offset by one.
    const valueMapping = parsedData.valueMappings.find(mapping => mapping.id === tokenId - 1);
    if (!valueMapping) {
      return {
        mappedValues: []
      }
    ...
    const mappedValues = valueMapping.values.map(([categoryId, optionId]) => {
      const category = parsedData.categories.find(cat => cat.id === categoryId);
  ...
  private fetchCollectionData(collectionId: string): Promise<any> {
    return fetch(`https://toniq.io/filter/${collectionId}.json`)
      .then(async (response) => {
        if (!response.ok) {
          return []
        }
