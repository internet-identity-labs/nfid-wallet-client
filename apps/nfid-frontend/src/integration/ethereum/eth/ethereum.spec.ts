...
const address = "0xAd984257f35B8dD8BF4154ccCfEDDE229CD1DC89"

describe("EthereumService", () => {
  jest.setTimeout(50000)

  it.skip("should be address", async () => {
    const response = await ethereumService.getAddress(idA)
  ...
  it.skip("should be ETH balance", async () => {
    //59936866100220000n
    const ethBalance = await ethereumService.getBalance(address)
    expect(ethBalance).toBe(BigInt(59936722671726420))
  })

  it.skip("should send ckETH to eth", async () => {
    const tr = await ethereumService.convertFromCkEth(address, "0.03", idA)
    console.log(tr)
  })

  it.skip("should send eth to ckETH", async () => {
    const tr = await ethereumService.convertToCkEth(idA, "0.009000070045", {
