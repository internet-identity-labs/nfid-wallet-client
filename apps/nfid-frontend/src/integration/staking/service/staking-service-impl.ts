...
export class StakingServiceImpl implements StakingService {
  async getStakedTokens(userId: string): Promise<Array<StakedToken>> {
    const snsTokens = await ftService.getTokens(userId)
      .then(tokens => tokens.filter(token => token.getTokenCategory() === Category.Sns))
    let promises =
      snsTokens.map(token => {
    ...
    return  await Promise.all(promises)
      .then(tokens => tokens.filter(token => token !== undefined))
  }
  ...
  async stake(token: FT, amount: string, delegation: SignIdentity): Promise<NFIDNeuron> {
    let root = token.getRootSnsCanister()
    if (!root) {
      throw new Error("Root Canister not found")
    }
    let amountInE8S = BigNumber(Number(amount))
    ...
    let createdNeuron = neurons.find((neuron) => bytesToHexString(neuron.id[0]!.id) === bytesToHexString(id.id))
    if (!createdNeuron) {
