...
export class DemoTransactions extends demoAppPage {
  ...
  async getAuthLogs() {
    await browser.waitUntil(
      async () => {
        let text = new Map(
          Object.entries(JSON.parse(await this.getAuthLogsLocator.getText())),
        )
        if (
          text.get("principal") != "" &&
          text.get("address") != "" &&
  ...
  async getTransferLogs() {
    await browser.waitUntil(
      async () =>
        new Map(
          Object.entries(
            JSON.parse(await this.getTransferLogsLocator.getText()),
          ),
        ).size > 0,
      {
        timeout: 20000,
    ...
    return new Map(
      Object.entries(JSON.parse(await this.getTransferLogsLocator.getText())),
    )
  }
  ...
  async selectProfile(profile: ProfileType) {
    return profile == ProfileType.public
      ? super.getPublicProfile
      : super.getAnonymousProfiles
  }
  ...
  public override async loginUsingIframe(profile: ProfileType) {
    ...
    ).waitForDisplayed({ timeout: 50000 })
