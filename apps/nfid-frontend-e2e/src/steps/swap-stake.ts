...
const StakedTokenDetailsBalances = {
...
When(
  /^Verifying that the (Swap|Stake) transaction is success$/,
  async (action: string) => {
    if (action == "Swap") {
...
When(/^User clicks the (Swap|Stake) tokens button$/, async (action: string) => {
  ...
  const expectedTitle =
    action == "Swap" ? "Swap successful" : "Staked successfully"
...
When(
  ...
  async (tokenRole: string, token: string) => {
    await Assets.getChooseTokenModalButton(tokenRole).then(async (it) => {
      await it.waitForClickable()
      await it.click()
    })
    await Assets.getSearchTokenInputField(tokenRole).then(async (it) => {
      await it.waitForClickable()
      await it.setValue(token.replace(/^\$/, ""))
    })
    await Assets.getTokenByNameFromList(
      tokenRole,
      token.replace(/^\$/, ""),
    ).then(async (it) => {
...
When(
  /^User sets amount to (Swap|Stake) to (.*)$/,
  async (action: string, amount: string) => {
    ...
    if (action == "Swap") {
      ...
      SwapBalances.currentTargetTokenBalance = parseFloat(
        (await Assets.getTargetTokenBalance.getText()).replace(/[^\d.]/g, ""),
      )
...
When(
  ...
  async (targetToken: string, sourceToken: string) => {
    const BALANCE_TOLERANCE = 0.00000002
    ...
    await browser.waitUntil(
      async () => {
        ...
        expectedSourceTokenBalance =
          Math.floor(
            (SwapBalances.currentSourceTokenBalance -
              SwapBalances.sourceTokenAmountToSwap) *
              1e8,
          ) / 1e8
        expectedTargetTokenBalance =
          Math.floor(
            (SwapBalances.currentTargetTokenBalance +
              SwapBalances.expectedTargetTokenAmount) *
              1e8,
          ) / 1e8
        ...
        return (
          Math.abs(expectedSourceTokenBalance - actualSourceTokenBalance) <
            BALANCE_TOLERANCE &&
          Math.abs(expectedTargetTokenBalance - actualTargetTokenBalance) <
            BALANCE_TOLERANCE
        )
      },
      {
...
When(
  "User moves pointer for {int} to set the Lock Time to {lockTime}",
    ...
    if ((await Staking.lockTimePeriod.getAttribute("value")).trim() != time) {
      await browser.waitUntil(
        async () => {
          ...
          return (
            (await Staking.lockTimePeriod.getAttribute("value")).trim() == time
          )
        },
...
When(
  "User verifies total staking balances and staked {token} token balances were changed correctly",
  async (tokenName: string) => {
    ...
    StakingTotalBalances.expectedStakingUSDBalance =
      StakingTotalBalances.currentStakingUSDBalance +
      StakingTotalBalances.amountToStake * tokenUSDPrice

    StakingTotalBalances.expectedStakedUSDAmount =
      StakingTotalBalances.currentStakedUSDAmount +
      StakingTotalBalances.amountToStake * tokenUSDPrice
    ...
    StakedTokenBalances.expectedStakedAmount =
      StakedTokenBalances.currentStakedAmount +
    ...
    await browser.waitUntil(
      async () => {
        ...
        const stakedTokenAmount = Number(
          (await Staking.stakedToken(tokenName).stakedAmount.getText()).replace(
            /[^\d.]/g,
            "",
          ),
        )
        ...
        const stakedTokenRewards = Number(
          (await Staking.stakedToken(tokenName).rewards.getText()).replace(
            /[^\d.]/g,
            "",
          ),
        )
    ...
    await browser.waitUntil(
      async () => {
        ...
        StakedTokenDetailsBalances.expectedStakingBalance =
          StakedTokenDetailsBalances.currentStakingBalance +
          StakingTotalBalances.amountToStake

        StakedTokenDetailsBalances.expectedStakedAmount =
          StakedTokenDetailsBalances.currentStakedAmount +
          StakingTotalBalances.amountToStake
...
When(
  "System saves current user's total staking values and staked {token} token values",
  async (tokenName: string) => {
...
When(
  "System saves current USD price of {token} token",
  async (tokenName: string) => {
    tokenUSDPrice = Number(
      (await (await Assets.tokenUSDPrice(tokenName)).getText()).replace(
        /[^\d.]/g,
        "",
      ),
    )
...
When(
  "User goes to details of the staked token with name {token}",
  async (tokenName: string) => {
    await Assets.waitUntilElementsLoadedProperly(
      Staking.stakedToken(tokenName).detailsButton,
      Staking.StakedTokenTransaction("Locked", tokenName)
...
When(
  "User verifies values are correct in the first transaction of {token} in {word} table",
  async (tokenName: string, tableType: "Locked" | "Unlocking") => {
    const actionType: "lockTime" | "unlockTime" =
      tableType == "Locked" ? "lockTime" : "unlockTime"

    StakingTransactionDetails.transactionID =
      await Staking.StakedTokenTransaction(
        tableType,
        tokenName,
      ).firstTransactionTokenID.getText()
...
When(
  "User click on the first row of the {word} table",
  async (tableType: "Locked" | "Unlocking") => {
    await Staking.StakedTokenTransaction(tableType).getFirstRowInTable.click()
  },
)

When(
  "User finds by ID and clicks on the transaction in {word} table",
  async (tableType: "Locked" | "Unlocking") => {
    await Staking.StakedTokenTransaction(
      tableType,
      StakingTransactionDetails.transactionID,
...
When(
  /^User verifies details of the (Locked|Unlocking) staking transaction are correct on the side bar$/,
  async (tableType: string) => {
    ...
    await softAssertAll(
      ...
      [
        async () => {
          expect(StakingTotalBalances.amountToStake).toEqual(
            Number(
              (await Staking.sidePanel().initialStake.getText()).replace(
                /[^\d.,]/g,
                "",
              ),
            ),
      ...
      [
        async () => {
          expect(StakingTotalBalances.amountToStake).toEqual(
            Number(
              (await Staking.sidePanel().totalValue.getText()).replace(
                /[^\d.,]/g,
                "",
              ),
            ),
      ...
      [
        async () => {
          expect(StakingTransactionDetails.createdDate).toEqual(
            formatDate(Date.now()),
          )
        },
...
When(/^User click the (?:Start|Stop) unlocking button$/, async () => {
  await Staking.sidePanel().startActionButton.click()
})

When(
  /^User verifies that the staking transaction is moved to the (Locked|Unlocking) table$/,
  async (tableType: "Locked" | "Unlocking") => {
    await browser.waitUntil(
      async () => {
        const transaction: WebdriverIO.Element =
          await Staking.StakedTokenTransaction(
            tableType,
          ).getAllTableTransactionsIDs.find(async (el) => {
            return (
              (await el.getText()) == StakingTransactionDetails.transactionID
            )
          })
      ...
      {
        timeout: 30000,
        timeoutMsg: `Transaction with ID '${StakingTransactionDetails.transactionID}'
