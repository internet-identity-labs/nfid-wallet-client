Feature: Tokens tab
  ...
  Scenario Outline: User checks his tokens are displayed and checks the ability to manage his tokens
    ...
    And Verifying that the token <token2> is displayed
    When User opens Token options dialog window of <token2>
    And User clicks the Hide token option button
    Then Verifying that the token <token2> is not displayed
    ...
    Then Verifying that the token <token2> is displayed
    Examples:
      ...
      | Chain Fusion | ckPEPE   | 200000276   | ckPEPE            | ckPEPE | 953 ckPEPE    |
