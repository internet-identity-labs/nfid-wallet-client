...
export class HomePage extends Page {
  ...
  public async authenticateWithGoogle() {
    await this.googleAuthButton
      .waitForDisplayed({ timeout: 6000, timeoutMsg: "Google auth button is missing!" });
    await this.googleAuthButton.waitForClickable();
    await this.googleAuthButton.click();
  }

  public async pickGoogleAccount() {
    await this.accountPicker.waitForDisplayed({ timeout: 7000, timeoutMsg: "Google Account is missing!" });
    await this.accountPicker.waitForClickable();
    await this.accountPicker.click();
  }

  public async captchaPass() {
    await this.captchaImage
      .waitForDisplayed({ timeout: 18000, interval: 1000, timeoutMsg: "Captha is failed to appear" });
  ...
  public async justLogMeIn() {
    await $("#just-log-me-in")
      .waitForDisplayed({ timeout: 30000, timeoutMsg: "Just Log Me In option is missing" });
