...
import { AccountBalance, AppBalance, TokenBalanceSheet } from "src/features/fungable-token/types";
import { IconSvgBTC } from "@nfid-frontend/ui";
import { E8S } from "@nfid/integration/token/icp";
import { btcWallet as btcAPI, loadProfileFromLocalStorage, replaceActorIdentity } from "@nfid/integration";
...
export const getBtcBalance = async (): Promise<TokenBalanceSheet> => {
  ...
  if (!address) {
    ...
    principal = await getWalletDelegation(profile?.anchor)
      .then((l) => l.getPrincipal().toText());
  }
...
export const computeSheetForRootAccount = async (balance: ChainBalance, address: string | undefined, principalId: string): Promise<TokenBalanceSheet> => {
  ...
  const appBalance: AppBalance = { accounts: [rootAccountBalance], appName: "NFID", tokenBalance };
  return {
    applications: {
      "NFID": appBalance
    },
