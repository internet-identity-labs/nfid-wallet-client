...
export const CANDID_UI_CANISTER = "a4gq6-oaaaa-aaaab-qaa4q-cai"
...
class InterfaceFactoryService {
  public async getInterfaceFactory(
    ...
    const candidFile: string | undefined = await this.getCandidFile(
      canisterId,
      agent as never,
    )
    if (!candidFile) {
  ...
  private async transformDidToJs(
    candid: string,
    agent: HttpAgent,
  ): Promise<string> {
