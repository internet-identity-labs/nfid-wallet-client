...
import { fetchAllDevices, addDevice } from "frontend/integration/internet-identity";
...
export class SyncDeviceIIService {
  ...
  async isEmailDeviceOutOfSyncWithII(): Promise<boolean> {
    ...
    if(profile.wallet === RootWallet.II && profile.email) {
      const emailDevice = profile.accessPoints.find((d) => d.deviceType === DeviceType.Email);

      if(!emailDevice) {
        return false;
      }

      const iiDevices = await fetchAllDevices(BigInt(profile?.anchor))
        .then(devices => mapIIDevicesToIDevices(devices))

      const { delegationIdentity } = authState.get()
      const isAuthentificatedII = iiDevices.find((device) => device.principal === delegationIdentity?.getPrincipal().toText())

      if(!isAuthentificatedII) {
        return false;
      }

      const isEmailDeviceInII = iiDevices.find((device) => device.principal === emailDevice?.principalId)
