...
> **Machines:** Root Auth, Email Flow, NFID Auth Wrapper, 3rd-Party Auth, Transfer Modal
...
> Root Auth and Email Flow are also invoked by the Embed and IdentityKit machines — see their respective specs.
...
```
AuthenticationMachine (invokes root) → End (final, carries authSession)
```

---

## 4. Third-Party Auth Machine (IDP / Wallet SDK)
...
└─ GetAppMeta: Fetch → Done
        │ both done
        ▼
AuthenticationMachine (invokes root auth)
        │ done
        ▼
