...
class SwapService {
  async getSwaps(source: string, target: string): Promise<Map<SwapName, Shroff>> {
    const kongShroff = await new KongShroffBuilder().withTarget(target).withSource(source).build()
    const icpSwapShroff = await new IcpSwapShroffBuilder().withTarget(target).withSource(source).build()
