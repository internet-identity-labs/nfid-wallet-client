...
export class NftMemeCake extends NFTExt {
  async getDetails(): Promise<NFTDetails> {
    ...
    return new NftMemeCakeDetails(nftResponse.data.collection, nftResponse.data.token)
  }
}
...
class NftMemeCakeDetails extends NFTDetailsImpl {
  ...
  async getTransactions(from: number, to: number): Promise<NFTTransactions> {
    const response: TransactionApiResponse = await fetch(`https://memecake.io/api/nft/token/${this.collection.collectionOnChainPublicAddress + "-" + this.token.tokenIndex}/activities`)
      .then((response) => response.json())
