import { Balance, RosettaBalance, TransactionHistory, XdrUsd } from "./rosetta_interface"
...
export async function mapToTransactionHistory(response: Response): Promise<TransactionHistory> {
  return await response.json().then((data) => camelizeKeys(data) as TransactionHistory)
}
