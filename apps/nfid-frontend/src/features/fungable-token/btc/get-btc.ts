...
import { btcWallet as btcAPI, loadProfileFromLocalStorage, replaceActorIdentity, setProfile } from "@nfid/integration";
...
export const getBtcTransactionHistory = async (): Promise<BtcTxs> => {
  ...
  return { sendTransactions,receivedTransactions, walletAddress: addressPrincipal , btcAddress: address };
};

export const getTransactions = async (type: string, address: string): Promise<TransactionRow[]> => {
  return await BtcAsset.getFungibleActivityByTokenAndUser({
    address,
    direction: type === "send" ? "from" : "to"
  }).then((tss) => {
    return tss.activities.map(tx =>
      ({
        type: tx.from === address.toLowerCase() ? "send" : "received",
        asset: "BTC",
        quantity: Number(tx.price) / E8S,
        date: format(new Date(Number(tx.date) * 1000), "MMM dd, yyyy - hh:mm:ss aaa"),
        from: tx.from,
        to: tx.to
      } as TransactionRow)
    );
