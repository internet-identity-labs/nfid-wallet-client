...
export const SendTransactionService = async ({ authSession }: ConnectAccountServiceContext, event: { type: string; data: RPCMessage },
) => {
  ...
  const message = Object.keys(rawMessage).reduce(
    (acc, key) => ({ ...acc, ...(rawMessage[key] ? { [key]: rawMessage[key] } : {}) }),
    {}
  )
