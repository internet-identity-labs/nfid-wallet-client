...
import {actor, exchangeRateService, hasOwnProperty, ICRC1TypeOracle} from "@nfid/integration";
...
class ShroffImpl implements Shroff {
  ...
  constructor(poolData: PoolData,
    ...
    this.swapPoolActor = actor<SwapPool>(
      poolData.canisterId,
      SwapPoolIDL,
    )
    this.source = source;
  ...
  async getQuote(amount: number): Promise<Quote> {
    const amountDecimals = new BigNumber(amount).multipliedBy(10 ** this.source.decimals)
      .minus(calculateWidgetFee(amount, this.source.decimals)).minus(Number(this.source.fee))
    ...
    const targetUSDPricePromise = exchangeRateService.usdPriceForICRC1(this.target.ledger)
    const sourceUSDPricePromise = exchangeRateService.usdPriceForICRC1(this.source.ledger)
    const quotePromise = this.swapPoolActor.quote(args) as Promise<Result>

    const [targetUSDPrice, sourceUSDPrice, quote] = await Promise.all([targetUSDPricePromise, sourceUSDPricePromise, quotePromise])

    if (hasOwnProperty(quote, "ok")) {
      return new QuoteImpl(amount, quote.ok as bigint, this.source, this.target, targetUSDPrice, sourceUSDPrice)
    }
...
export class ShroffBuilder {
  ...
  public async build(): Promise<Shroff> {
    ...
    const [poolData, icrc1canisters]: [PoolData, ICRC1TypeOracle[]] = await Promise.all([
      icpSwapService.getPoolFactory(this.source, this.target),
      icrc1OracleService.getICRC1Canisters()
    ])

    const st: ICRC1TypeOracle[] = icrc1canisters.filter((icrc1) => icrc1.ledger === this.source
      || icrc1.ledger === this.target)
