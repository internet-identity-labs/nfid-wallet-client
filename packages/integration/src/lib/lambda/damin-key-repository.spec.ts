import {domainKeyStorage, getFromStorage, saveToStorage} from "./damin-key-repository";
...
describe('Storage Functions', () => {
  ...
  it('saveToStorage saves data correctly', () => {
    ...
    const storedData = JSON.parse(localStorageMock.getItem(domainKeyStorage) || '{}');
  ...
  it('getFromStorage throws error for expired data', () => {
    ...
    expect(() => getFromStorage('key3')).toThrowError("Value for key 'key3' has expired.");
  });

  it('getFromStorage throws error for missing data', () => {
    expect(() => getFromStorage('nonExistentKey')).toThrowError("Value for key 'nonExistentKey' not found.");
  });
});
