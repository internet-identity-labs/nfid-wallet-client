...
When(/^Verifying the balance of (.*) token and (.*) token has changed correctly$/, async (targetToken: string, sourceToken: string) => {
  await browser.waitUntil(async () => {
    ...
    await browser.pause(2000)
    return Math.floor((currentSourceTokenBalance - sourceTokenAmountToSwap) * 1e8) / 1e8 ==
      (await (await Assets.tokenBalance(sourceToken)).getText()).replace(/[^\d.]/g, "")
      && Math.floor((currentTargetTokenBalance + expectedTargetTokenAmount) * 1e8) / 1e8 ==
      (await (await Assets.tokenBalance(targetToken)).getText()).replace(/[^\d.]/g, "")
  }, {
