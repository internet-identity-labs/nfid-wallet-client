...
import {deleteFromStorage, getFromStorage, saveToStorage,} from "./domain-key-repository"
...
export async function getGlobalKeysThirdParty(
  ...
  const delegationJSON = await getDelegationChain(toHexString(sessionPublicKey), targets, identity, maxTimeToLive)
  saveToStorage(
    origin,
...
export async function getGlobalKeys(
  ...
  const delegationJSON = await getDelegationChain(sessionKey.toJSON()[0], targets, identity)
...
export async function ecdsaGetAnonymous(
  ...
  const principal = await im.get_root_by_principal(identity.getPrincipal().toString())
...
async function getDelegationChain(publicSessionKey: string, targets: string[], delegationIdentity: DelegationIdentity, delegationTtl?: number) {
  ...
  if (canisterKeyPairResponse.key_pair.length === 0) {
    ...
    privateKey = decrypt(canisterKeyPairResponse.key_pair[0]!.private_key_encrypted, "utf8", await getEncryptionKey2(delegationIdentity));
...
async function getAnonEncryptionKey(identity: DelegationIdentity, domain: string) {
  // authStorage.set()
...
async function fetchAnonKey(request: Record<string, any>): Promise<any> {
  const signUrl = ic.isLocal ? `/ecdsa_get_anonymous_seed` : "https://ia15v0pzlb.execute-api.us-east-1.amazonaws.com/dev/ecdsa_get_anonymous_seed"
...
async function fetchECDSAKey(request: Record<string, any>): Promise<any> {
  const signUrl = ic.isLocal ? `/ecdsa_encryption_key` : "https://ia15v0pzlb.execute-api.us-east-1.amazonaws.com/dev/ecdsa_encryption_key"
  const response = await fetch(signUrl, {
    method: "POST",
    headers: {"Content-Type": "application/json"},
    body: JSON.stringify(request),
...
async function registerNewKeyPair(delegationIdentity: DelegationIdentity) {
  ...
  const encryptedPrivateKey = encrypt(ecdsaKeyPair.privateKey, "utf8", await getEncryptionKey2(delegationIdentity));
