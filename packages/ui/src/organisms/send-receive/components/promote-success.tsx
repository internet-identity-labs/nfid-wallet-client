import clsx from "clsx"
import { FC } from "react"

import { Button, H5 } from "@nfid-frontend/ui"

import { SendStatus } from "frontend/features/transfer-modal/types"

import { EarnAnimation } from "./earn-animation"
import { ChainId } from "@nfid/integration/token/icrc1/enum/enums"

export interface PromoteSuccessProps {
  title: string
  subTitle?: string
  chainId: ChainId
  onClose?: () => void
  assetImg?: string
  duration?: number
  isOpen: boolean
  status: SendStatus
  error?: string
}

export const PromoteSuccessUi: FC<PromoteSuccessProps> = ({
  title,
  subTitle = "0.00 USD",
  chainId,
  onClose,
  assetImg,
  duration = 5,
  isOpen,
  status,
  error,
}) => {
  return (
    <div
      id={"success_window_3"}
      className={clsx(
        "text-black dark:text-white text-center w-full h-full",
        "px-5 pb-5 pt-[18px] absolute left-0 top-0 z-[3]",
        "flex flex-col justify-between bg-white dark:bg-darkGray",
        !isOpen && "hidden",
      )}
    >
      <div className={clsx("text-center", { "mb-[50px]": !!error })}>
        <H5 className="mt-5 text-xl !font-bold leading-6 dark:text-white">
          {status === SendStatus.FAILED
            ? "Transaction failed"
            : status === SendStatus.COMPLETED
              ? "Promotion successful"
              : "Promoting"}
        </H5>
        <p className="h-5 mt-3 text-sm leading-5">
          {status === SendStatus.FAILED
            ? "Your assets are still in your wallet."
            : status === SendStatus.COMPLETED
              ? ""
              : `This usually takes about ${duration} seconds. `}
        </p>
      </div>
      <div className="relative flex items-center justify-center w-full">
        <EarnAnimation assetImg={assetImg!} status={status} chainId={chainId} />
      </div>
      <div className="relative z-20">
        <p className="text-sm leading-[25px] font-inter" id="title">
          {title}
        </p>
        <p className="text-xs text-gray-500 leading-[18px]" id="subTitle">
          {subTitle}
        </p>
        {error && (
          <div className="text-sm text-red-600 mt-[20px]">
            Something went wrong with promotion. <br />
            Please try again later.
          </div>
        )}
        <Button
          type="primary"
          block
          className={clsx(error ? "mt-[20px]" : "mt-[30px]")}
          onClick={onClose}
        >
          Done
        </Button>
      </div>
    </div>
  )
}
