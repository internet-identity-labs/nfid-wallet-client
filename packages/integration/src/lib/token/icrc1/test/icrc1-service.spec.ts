...
describe("ICRC1 suite", () => {
  ...
  it('should filter', async function () {
    ...
    const filteredCanisters = await icrc1StorageService.getICRC1FilteredCanisters(principal.toText(), "Ch")
    expect(filteredCanisters.length).toEqual(1)
    expect(filteredCanisters[0].name).toEqual("Chat")
    const filteredCanisters2 = await icrc1StorageService.getICRC1FilteredCanisters(principal.toText(), "CH")
    expect(filteredCanisters2.length).toEqual(1)
    expect(filteredCanisters2[0].name).toEqual("Chat")
    const filteredCanisters3 = await icrc1StorageService.getICRC1FilteredCanisters(principal.toText(), "C")
  ...
  it('should return active', async function () {
    ...
    const filteredCanisters = await icrc1StorageService.getICRC1ActiveCanisters(principal.toText())
    expect(filteredCanisters.length).toEqual(1)
