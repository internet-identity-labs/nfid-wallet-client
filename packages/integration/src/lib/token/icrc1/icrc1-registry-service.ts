...
export class Icrc1RegistryService {
  ...
  storeICRC1Canister(ledger: string,
                     state: State): Promise<void> {
    return iCRC1Registry.store_icrc1_canister(
      ledger, mapStateTS(state)
    )
  }
  ...
  async removeICRC1Canister(
    ...
    const allUsersCanisters = await iCRC1Registry.get_canisters_by_root(principal)
