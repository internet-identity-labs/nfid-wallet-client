...
import {_SERVICE as ICRCIndex, GetAccountTransactionsArgs, TransactionWithId, Transfer,} from "../_ic_api/index-icrc1.d"
...
export async function getICRC1DataForUser(
  ...
  return getICRC1Data(canisters.map(l=>l.ledger), publicKey)
}
 ...
 */
export async function getICRC1HistoryDataForUser(rootPrincipalId: string,
                                                 publicKey: string,
                                                 maxResults: bigint,
                                                 blockNumberToStartFrom: bigint | undefined
): Promise<Array<ICRC1IndexData>> {
  const canisters = await getICRC1Canisters(rootPrincipalId)
  const indexedCanisters = canisters.filter((canister) => canister.index.length > 0)
  return getICRC1IndexData(indexedCanisters, publicKey, maxResults, blockNumberToStartFrom)
}
...
export async function addICRC1Canister(canisterId: string, indexCanisterId: string | undefined): Promise<void> {
  ...
  return Promise.all(
    canisters.map(async (canisterId) => {
      ...
      if (hasOwnProperty(response, "Err")) {
        console.warn("Error " + response.Err + " getting account transactions for canister: " + canisterId)
        return {transactions: [], oldestTransactionId: undefined}
      }

      if (hasOwnProperty(response, "Ok")) {
        const ledgerData = await getICRC1Data([canisterId.ledger], publicKeyInPrincipal);
        return {
          transactions: mapRawTrsToTransaction(response.Ok.transactions, publicKeyInPrincipal, ledgerData[0].symbol),
...
function mapRawTrsToTransaction(rawTrss: Array<TransactionWithId>, ownerPrincipal: string, symbol: string): Array<TransactionData> {
  const filtered = rawTrss.filter((rawTrs) =>
    rawTrs.transaction.transfer.length !== 0
  )
  ...
  return filtered.map((rawTrs) => {
    ...
    const type = ownerPrincipal === trs.from.owner.toText() ? "sent" : "received"
