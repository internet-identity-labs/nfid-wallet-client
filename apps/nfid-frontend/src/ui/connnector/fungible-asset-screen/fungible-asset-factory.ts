import {AssetErc20Config, AssetFilter, AssetNativeConfig, TokenConfig,} from "src/ui/connnector/types"
...
export class FungibleAssetFactory extends ConnectorFactory<TokenStandards, FungibleAssetConnector<AssetNativeConfig | AssetErc20Config>> {
  ...
  getCacheKey(key: TokenStandards, functionToCall: Function, args: any[]): string {
    return functionToCall.name + key
      + (args[0]! as AssetFilter[]).reduce((acc, {principal}) => `${acc}${principal}`, "")
  }

  getFunctionToCall(connector: FungibleAssetConnector<AssetNativeConfig | AssetErc20Config>): Function {
