...
Feature: Registration and login using Google
  ...
  Scenario Outline: User wants to register and login with Google
    ...
    Then I wait on element "iframe" for 60000ms to be displayed
    When I click on the selector "iframe"
    When I focus the last opened window
    Then I wait on element "[role='link']:first-child" for 60000ms to exist
    ...
    Then I wait on element "#profile-icon" for 60000ms to be displayed
    When I click on the selector "#profile-icon"
    Then I wait on element "#logout" for 60000ms to exist
