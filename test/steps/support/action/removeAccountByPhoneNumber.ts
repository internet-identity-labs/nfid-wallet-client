...
export default async () => {
  if(LAMBDA_IDENTITY && IDENTITY_MANAGER_CANISTER_ID_DEV) {
    ...
    let actor = Actor.createActor(idlFactory, {agent, canisterId: IDENTITY_MANAGER_CANISTER_ID_DEV});
    await actor.remove_account_by_phone_number();
  } else {
    throw Error(`LAMBDA_IDENTITY or IDENTITY_MANAGER_CANISTER_ID_DEV is not defined.`);
  }
}
