...
class RecoveryPage extends HomePage {
  ...
  public async recoverAccountWithThePhrase(phrase: string) {
    await this.phraseTextarea.waitForDisplayed({
      timeout: 6000,
      timeoutMsg: "Recovery Phrase input is missing!",
    })
    await this.phraseTextarea.setValue(phrase)
    await this.confirmationCheckMark.click()
    await this.recoveryButton.waitForDisplayed({
      timeout: 3000,
      timeoutMsg: "Recovery Butotn is missing",
    })
