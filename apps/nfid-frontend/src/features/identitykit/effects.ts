...
export const sendResponseEffect = async (context: any, event: any) => {
  ...
  const responseData = event.output ?? event.data

  if (responseData instanceof Error || responseData instanceof GenericError) {
    parent.postMessage(
      {
        origin: context.activeRequest.origin,
        jsonrpc: context.activeRequest.data.jsonrpc,
    ...
    parent.postMessage(responseData, request.origin)
  }
}
